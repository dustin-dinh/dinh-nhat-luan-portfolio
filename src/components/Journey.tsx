      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="mb-12 space-y-3">
          <div className="inline-flex items-center gap-2 rounded-md border border-[#A98A8A]/30 bg-[#715A5A]/25 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[#E5D5D5]">
            <Milestone className="h-3.5 w-3.5 text-[#A98A8A]" />
            {t.journey.sectionTag}
          </div>
          <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
            {t.journey.title}
          </h2>
          <p className="max-w-2xl text-base text-[#D3DAD9]">{t.journey.subtitle}</p>
        </div>
        ...
        <div className="relative border-l-2 border-[#A98A8A]/30 pl-6 sm:pl-8 space-y-10">
          {t.journey.steps.map((step, index) => (
            <div key={step.title} className="relative group">
              ...
              <div className="rounded-2xl border border-[#D3DAD9]/12 bg-[#37353E] p-6 shadow-xl transition-all group-hover:border-[#A98A8A]/50">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="font-mono text-xs font-bold text-[#A98A8A] uppercase tracking-wider">
                    Step 0{index + 1} · {step.period}
                  </span>
